...
import {insertarEspecialidadSchema, modificarEspecialidadSchema} from "../schemas/especialidadSchema";

export const insertarEspecialidad = async (req : Request, res : Response) => {
    console.log('especialidadController : insertarEspecialidad');
    try {
        const { error } = insertarEspecialidadSchema.validate(req.body);
        if (error) {
...
export const modificarEspecialidad = async (req : Request, res : Response) => {
    ...
    try {
        const { id } = req.params;
        const { error } = modificarEspecialidadSchema.validate(req.body);
        if (error) {
            console.error(error.message);
            res.status(400).json(ResponseModel.error(error.message,400));
            return;
